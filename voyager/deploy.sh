...
#
#
# Deploys Voyager "the Pivotal Way"
#
#
...
mv deploy_tmp/config-templates/deploy.laravel.json deploy.json
rm -rf deploy_tmp
...
while [ $DEPLOY_FINISHED != 1 ]; do
  sleep 5
  [ ! -f "${DIR_NAME}/server.php" ] && DEPLOY_FINISHED=0 || DEPLOY_FINISHED=1
...
cp .env.example .env
sed -i 's,APP_URL=http://localhost,APP_URL='"$PUBLIC_SITE_URL"',g' .env
...
# Create a generic .htaccess/.htpasswd file in root for password restrictions
# ---------------------------------------------
echo '
...
echo -e "       2. Setup the CRON - `* * * * * cd ${DIR_NAME} && /usr/local/bin/php artisan schedule:run >> /dev/null 2>&1`"
